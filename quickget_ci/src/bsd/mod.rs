...
impl Distro for FreeBSD {
    ...
    async fn generate_configs() -> Vec<Config> {
        ...
        if let Some(page) = capture_page(FREEBSD_X86_64_RELEASES).await {
            for capture in freebsd_regex.captures_iter(&page) {
                ...
                futures.push(spawn(async move {
                    ...
                    let checksums = capture_page(&checksum_url).await;
                    FREEBSD_EDITIONS
                        .iter()
                        .map(|edition| {
        ...
        if let Some(page) = capture_page(FREEBSD_AARCH64_RELEASES).await {
            for capture in freebsd_regex.captures_iter(&page) {
                let release = capture[1].to_owned();
                futures.push(spawn(async move {
        ...
        let releases = futures::future::join_all(futures).await;
        releases.into_iter().flatten().flatten().collect::<Vec<Config>>()
    }
}
